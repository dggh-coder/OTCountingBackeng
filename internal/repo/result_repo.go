...
func (r *ResultRepo) ListBySession(ctx context.Context, sessionID int64) ([]service.ResultRecord, error) {
	q := querier(ctx, r.DB)
	rows, err := q.QueryContext(ctx, `SELECT session_id, employee_id, date_label, rate20_minutes, rate20_rounded_hours, rate15_minutes, rate15_rounded_hours, calculated_at FROM session_result WHERE session_id=$1 ORDER BY employee_id`, sessionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.SessionID, &r.EmployeeID, &r.DateLabel, &r.Rate20Minutes, &r.Rate20RoundedHours, &r.Rate15Minutes, &r.Rate15RoundedHours, &r.CalculatedAt); err != nil {
			return nil, err
		}
		out = append(out, r)
	}
