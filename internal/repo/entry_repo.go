...
func (r *EntryRepo) ReplaceBySession(ctx context.Context, sessionID int64, entries []service.SessionEntry) error {
	...
	for _, e := range entries {
		if _, err := q.ExecContext(ctx, `INSERT INTO time_entry (id, session_id, employee_id, entry_type, start_time, end_time) VALUES ($1::uuid,$2,$3,$4,$5::time,$6::time)`, e.ID, sessionID, e.EmployeeID, e.EntryType, e.StartTime, e.EndTime); err != nil {
			return err
		}
	}
	return nil
...
func (r *EntryRepo) ListBySession(ctx context.Context, sessionID int64) ([]service.SessionEntry, error) {
	q := querier(ctx, r.DB)
	rows, err := q.QueryContext(ctx, `SELECT session_id, id::text, employee_id, entry_type, to_char(start_time, 'HH24:MI'), to_char(end_time, 'HH24:MI') FROM time_entry WHERE session_id=$1`, sessionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
